...
Class Action {
	...
	function signup(){
		...
		if($chk > 0){
			...
			// if($save == false)
			// {
			// 	// $sql = $this->db->query("SELECT email from alumnus_bio");
			// 	// $email = $sql->fetch_array();
			// 	$to = $email;
	...
	function update_donation_sts(){
		extract($_POST);
		$update = $this->db->query("UPDATE donations set is_active = $status where donate_id = $id");
		if($update)
			return 1;
	}
	...
	function save_career(){
		...
		$data .= ", user_id = '{$_SESSION['login_id']}' ";
			...
			// 	   $to = $email;
            //        $subject = "Notification";
            //        $txt = "New JOB Added Please Check The following Web Site '<br>'
            //        http://localhost/alumni/index.php?page=careers";
            //        mail($to,$subject,$txt);
			// }
		}else{
	...
	function save_donation(){
		...
		$data .= ", upi_pay = '$upi_pay'";
		$data .= ", acc_no = '$acc_no'";
		$data .= ", amount = '$amount'";
		$data .= ", connected_to = '$connected_to'";
		$data .= ", credited_to = '$credited_to'";
		$data .= ", utr_no = '$utr_no'";
		$data .= ", donate_desc = '$donate_desc'";

		if(!empty($id)){
			$save = $this->db->query("Update donations set ".$data." where donate_id = ".$id);
		}
		if($save)
		return 1;
	}
	...
	{
		extract($_POST);
		$delete = $this->db->query("DELETE FROM donations where donate_id = ".$id);
		if($delete){
			return 1;
		}
	}
	...
	function save_event(){
		...
		if(empty($id)){
			...
			// 	$sql = $this->db->query("SELECT email from alumnus_bio");
			// 	$email = $sql->fetch_array();
			// 	   $to = "rakesh.yerrabolu@yahoo.com";
            //        $subject = "Notification";
            //        $txt = "New Event Added Please Check The following Web Site '<br>'
            //        http://localhost/alumni/index.php?page=home";
            //        mail($to,$subject,$txt);
			// }
		}else{
